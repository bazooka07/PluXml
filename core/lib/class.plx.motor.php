...
class plxMotor {
	 ...
	 *
	 * @return	null
	 * @author	Anthony GUÉRIN, Florent MONTHEL, Stéphane F, J.P. Pourrez aka bazooka07
	 **/
	public function prechauffage() {
		...
		# On initialise la future valeur à affecter à $this->motif pour la recherche d'articles
		# A ajuster selon le mode
		$artQuery = array(
			'artId' => '\d{4}',
			'cats'	=> '(?:home,|pin,|\d{3},)*(?:'.$this->activeCats.')(?:,\d{3})*',
			'auteur'=> '\d{3}',
			'date'	=> '\d{12}',
			'url'	=> '[\w-]+',
			'ext'	=> 'xml'
		...
		if(
			$this->__prechauffage_homestatic() or
			$this->__prechauffage_blog($artQuery) or (
				!empty($this->get) and (
					$this->__prechauffage_article($artQuery) or
					$this->__prechauffage_categorie($artQuery) or
					$this->__prechauffage_tags($artQuery) or
					$this->__prechauffage_static() or
					$this->__prechauffage_archives($artQuery) or
					$this->__prechauffage_preview() or
					$this->__prechauffage_telechargement()
				)
			)
		) {
			# Hook plugins
	...
	public function parseCommentaire($filename) {
		...
		$com['user_agent'] = (!empty($iTags['user_agent'])) ? plxUtils::getValue($values[$iTags['user_agent'][0]]['value']) : '';
	...
	public function newCommentaire($artId,$content) {
		...
		if(
			strtolower($_SERVER['REQUEST_METHOD'])!= 'post' or
			empty(plxUtils::checkLanguage($this->aConf['default_lang'])) or
			empty(plxUtils::checkContent()) or
			($this->aConf['capcha'] AND (
				!isset($_SESSION["capcha_token"]) OR
				!isset($_POST['capcha_token']) OR
		...
		if($this->aConf['capcha'] == 0 OR $_SESSION['capcha'] == sha1($content['rep'])) {
			if(!empty($content['name']) AND !empty($content['content'])) { # Les champs obligatoires sont remplis
				...
				# On récupère l'adresse IP du contributeur
				$comment['ip'] = plxUtils::getIp();
				# On récupère le User-Agent du contributeur
...
<comment>
	...
	<ip>{$content['ip']}</ip>
	<user_agent>{$content['user_agent']}</user_agent>\n
XML_STARTS;
		foreach(explode(' ', 'author mail site content parent') as $field) {
			$value = plxUtils::cdataCheck($content[$field]);
			if(!empty($value)) {
				$value = "<![CDATA[$value]]>";
			}
			$xml .= <<< FIELD
...
XML_ENDS;
		...
		# On écrit ce contenu dans notre fichier XML
	 ...
	 **/
	public function urlRewrite($url='', $catUrl = true) {
		...
		# splits url into path, query and fragment parts
		...
		if(!empty($this->aConf['urlrewriting'])) {
			...
			if(!empty($matches[2])) {
				# drops any number
				# The fragment ($matches[3]) is not empty if the url linking to a comment.
				# The urls in the last artList  have a fragment, too.
				if(($this->mode == 'categorie') and $catUrl and preg_match('@\barticle\d{1,4}\b@', $matches[2])) {
					$categorie = $this->aCats[$this->cible];
					$result .=  preg_replace('@\barticle\d{1,4}\b@', $categorie['url'], $matches[2]);
				} else {
					$result .= preg_replace('@\b(article|categorie|static)\d{1,4}\b@', '$1', $matches[2]);
				}
			}
			# fragment
			if(!empty($matches[3])) {
			...
			# path
			$result .= (!empty($matches[1])) ? $matches[1] : 'index.php';
			# query
			if(!empty($matches[2])) {
				$result .= '?'.$matches[2];
			}
